...
function numMinMax(num: number, min = 0, max?: number) {
...
function parseObj(rawJson: any, def: ComeViewConfig): ComeViewConfig {
  ...
  (Object.keys(def.display) as (keyof ComeViewConfig['display'])[]).forEach(
    (key, i) => {
      if (!res.display[key]) {
  ...
  (Object.keys(def.color) as (keyof ComeViewConfig['color'])[]).forEach(
    (key, i) => {
      if (!res.color[key]) {
  ...
  (Object.keys(def.animeIn) as (keyof ComeViewConfig['animeIn'])[]).forEach(
    (key, i) => {
      if (!res.animeIn[key]) {
...
function generateOutlineShadow(size = 1, color = '#000') {
  const shadowList = [];
...
function generateDistCSS(config: ComeViewConfig) {
  ...
  if (animeInNameVal.length > 0) {
    css.push(`:root { --in-animation-name: ${animeInNameVal.join(',')}; }`);
  } else {
...
export const ComeViewSetting: React.FC<{
  copiedS: AppConfig;
}> = ({ copiedS }) => {
  ...
  useEffect(() => {
    ...
    return () => {
      if (configRef.current) {
        saveConfig(configRef.current).then(() => {
          sendChatApi('reload', {});
        });
      }
    };
        ...
        {config && (
          <div className="body">
              ...
              <div>
                <FormatWrap>
                  ...
                  <Btn2
                    title="ブラウザで開く"
                    onClick={() =>
                      invokeOrigin('open_in_browser', { url: viewURL })
                    }
                  >
            ...
            <Item>
              ...
              <SmallItemWrap>
                {(
                  Object.keys(
                    config.display
                  ) as (keyof ComeViewConfig['display'])[]
                ).map((key, i) => {
                  const item = config.display[key];
                  const prefix = 'display__' + key;
                  return (
                    <SmallItem key={i}>
                      <label htmlFor={prefix}>{item.displayName}</label>
            ...
            <Item>
              ...
              <SmallItemWrap>
                {(
                  Object.keys(config.color) as (keyof ComeViewConfig['color'])[]
                ).map((key, i) => {
            ...
            <Item>
              ...
              <SmallItemWrap>
                {(
                  Object.keys(
                    config.animeIn
                  ) as (keyof ComeViewConfig['animeIn'])[]
                ).map((key, i) => {
              ...
              <label htmlFor="custom_tag" className="title">
                ...
                <Btn2
                  onClick={() => {
                    ...
                    if ($elm)
                      ($elm as HTMLInputElement).value = config.customTag;
                  }}
                  className="warn"
                >
