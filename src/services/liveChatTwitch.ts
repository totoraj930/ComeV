...
export type TwitchMetaData = LiveChatMetaData;
...
export function generateBouyomiText(
  ...
  switch (item.type) {
    ...
    case 'Cheer': {
      res = config.twitch.cheer.replace(/\$\(Amount\)/g, item.bits + '');
      message = ttvMessageToString(item.message, false);
      break;
    }
    case 'Sub': {
      res = item.methods.prime ? config.twitch.subPrime : config.twitch.sub;
      break;
    }
    case 'SubMysteryGift': {
      res = config.twitch.subGift.replace(/\$\(GiftNum\)/g, item.num + '');
      break;
    }
  }
  const name = item.author.displayName || item.author.name || '';
  res = res.replace(/\$\(Message\)/g, message).replace(/\$\(Name\)/g, name);
  return res;
}
...
const dummyCheermotes = [
  {
    ...
    cheermote: {
      ...
      url: {
        ...
        light:
          'https://d3aqoihi2n8ty8.cloudfront.net/actions/cheer/light/static/100/4.png',
      },
      animated_url: {
        dark: 'https://d3aqoihi2n8ty8.cloudfront.net/actions/cheer/dark/animated/100/4.gif',
        light:
          'https://d3aqoihi2n8ty8.cloudfront.net/actions/cheer/light/animated/100/4.gif',
      },
    },
  },
];
...
export function generateDummy(type: TwitchChatItem['type']): TwitchChatItem {
  ...
  switch (type) {
    case 'Normal': {
      return {
        ...base,
        type: 'Normal',
        message: ['これはダミーです', dummyEmotes[0]],
      };
    }
    case 'Cheer': {
      return {
